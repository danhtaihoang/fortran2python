...
nrepeat = 300   # update hidden spin
nloop = 400     # predict W

# parameters:
#g=2.0 ; N0=20 ; Nh = 6 ; Nb = Nh ; fN2=4.0
g = sys.argv[1] ; N0 = sys.argv[2] ; Nh = sys.argv[3] ; Nb = sys.argv[4] ; ln2 = sys.argv[5] ;  nens = sys.argv[6]
print(g, N0, Nh, Nb, ln2, nens)
g = float(g) ; N0 = int(N0) ; Nh = int(Nh) ;  Nb = int(Nb) ; ln2 = float(ln2) ; nens = int(nens)
L0 = int(ln2*N0**2)+nens
#ln2 = N0**2
#L0 =int(ln2*nens)

#L0=int(943*nens) ## Gasper-data
...
ext_name = '%02d_%02d_%02d_%02d.dat'%(N0,Nh,Nb,nens)
...
##========================================================================================
S00 = np.ones((L0,N0))
def generate_config(W0,L0):
   for t in range(L0-1):
      h = np.sum(W0[:,:]*S00[t,:],axis=1)
      p = 1/(1+np.exp(-2*h))
      S00[t+1,:]= np.sign(p-np.random.rand(N0))
   return S00
##========================================================================================
## predict W:
##========================================================================================
...
def coupling_inference(i0):
    s = S.copy() ; s1 = S1[:,i0].copy()
    c_inv = C_inv.copy() ; ds = dS.T ; l=S.shape[0]
...
def update_hidden():  ## NOT parallel
    n = N ; n2 = N2 ; h0 = H0.copy() ; w=W.copy() ; s = S0.copy()
...
def hidden_cordinate():
    ...
    for i in range(N,N0):
        for j in range(N,N0):
            cost1[i,j]=np.sum(np.abs(S00[:L,i]-S0[:L,j]))
            cost2[i,j]=np.sum(np.abs(S00[:L,i]+S0[:L,j]))
...
if generate_data=='YES':
   ...
   S00=np.loadtxt('Gasper_sort-after-clean.txt',unpack=True)
   S00=S00.T
    
#------------------------------------------------------------------------------------------

S0=np.ones((L0,N2))
S0[0:L0,0:N] = S00[0:L0,0:N] # observed sequences

#initial hidden:
if Nb>0:
    S0[0:L0,N:N2] = np.sign(np.random.rand(L0,Nb)-0.5)
#print(S)

## observed part and initial hidden part:
## for ensembles:
L01 = L0/nens # len of each ensemble
rows_del_S = [i for i in range(L0) if (i+1)%L01==0]
S=np.delete(S0,rows_del_S,axis=0)
rows_del_S1 = [i for i in range(L0) if (i+1)%L01==1]
S1=np.delete(S0,rows_del_S1,axis=0)
L = S.shape[0]
...
for irepeat in range(nrepeat):
    ...
    if (nPC>1):
        ...
        #res=ftlib.coupling_prediction(s=S[0:L],st1=S1[0:L],ds=dS[0:L],m=M,c_inv=C_inv)
        #W = res[0] ; H0 = res[1] ; cost = res[2]
    ...
    ## likelihood:
    H=np.matmul(S[0:L,:],W.T)+H0
    #print(H[0:3,:])
    #for i in range(N2):
    #    P_inv[i] = np.mean(1/(1+np.exp(-2*S1[:,i]*H[:,i])))

    P_inv=np.mean((1/(1+np.exp(-2*S1[:,:]*H[:,:]))),axis=0)
    P_total = np.mean(P_inv)

    if irepeat%1 == 0:
        print(irepeat,cost_repeat[irepeat],MSE_oo,slope_oo,P_total)

    cost_out.write("%i %f %f %f %f %f \n" % (irepeat,cost_repeat[irepeat],cost_repeat_oo[irepeat],\
            MSE_oo,slope_oo, P_total))
...
##@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    if Nb>0:
        ...
        S_out=ftlib.update_hidden_ens(n1=N+1,w=W.T,h0=H0,s=S0,nens=nens) # use fortran lib
        S0[:,N:N2]=S_out[:,0:Nb]

...
def coupling_inference():
    ...
    iloop = 1 ; cost_tmp = np.empty(N2) ; cost_tmp[:]=100.
    W=np.zeros((N2,N2))

    while iloop < nloop and n>0:
        h_av = np.mean(h,axis=0)    # (n,)
        hs_av = np.matmul(ds[0:L,0:N2].T, h-h_av.T)/L  # (N,L) x (L,n) = (N,n)
        w_tmp = np.matmul(hs_av.T, C_inv[0:N2,0:N2])   # (n,N) x (N,N) = (n,N)
        h0 = h_av - np.matmul(w_tmp,M)                 # (n,) - (n,N)x(N,)
        h = np.matmul(s[0:L,0:N2],w_tmp.T) + h0        # (L,N) x (N,n) = (L,n)

        cost=np.mean((s1-np.tanh(h))**2,axis=0)

        for i in range(n-1,-1,-1):
            if cost[i] > cost_tmp[i]:
                i0=np.where(id==i)[0]
                W[i0,:]=w_tmp[i,:]
                s1=np.delete(s1,i,axis=1)
                h=np.delete(h,i,axis=1)
                cost_tmp = np.delete(cost_tmp,i,axis=0)

...
print("You are using %d CPUs" % nPC)

nrepeat = 10   # update hidden spin
nloop = 1000     # predict W

# parameters:
g=2.0 ; N0=100 ; fN2=2.0
...
## predict W:
##========================================================================================
W_all=np.empty((N2,N2)) ; cost = np.empty(N2) ; H0_all=np.empty(N2)
def coupling_inference(i0):
    h = S1[:,i0].copy() # initial value
    w1=np.empty((nloop,N2)) ; h0=np.empty(nloop)
    iloop=1 ; stop_iloop=0 ; cost1 = np.full(nloop+1,100.)
    while iloop < nloop and stop_iloop == 0:
        h_av =np.mean(h)
        hs_av = np.matmul(dS[0:L,:].T,h-h_av)/L
        w = np.matmul(hs_av[:],C_inv[:,:])
        h0[iloop] =h_av-np.sum(w*M)
        h[0:L] =h0[iloop]+np.matmul(S[0:L,:],w[:])
        cost1[iloop] = np.mean((S1[:,i0] - np.tanh(h)) ** 2)

        if cost1[iloop] > cost1[iloop-1] :
            stop_iloop = 1
    ...
    return W_all[i0,:],cost[i0],H0_all[i0]
##========================================================================================
## update hidden:
##========================================================================================
def update_hidden():
...
for irepeat in range(nrepeat):
    ...
    if (nPC>1) :
        ...
        res=pool.map(coupling_inference,list(range(N)))
        pool.close()

        W_all=np.array([r[0] for r in res])
        cost=np.array([r[1] for r in res])
        H0_all=np.array([r[2] for r in res])
    else:

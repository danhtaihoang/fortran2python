...
## predict W:
##========================================================================================
W_all=np.empty((N2,N2)) ; cost = np.empty(N2) ; H0_all=np.empty(N2) ; H0=np.empty(nloop)
#def coupling_inference(N,S1,C_inv):
def coupling_inference():
    S1 = np.copy(S[1:])
    for i0 in range(N2):
        #W0 = W0_all[i0,:] ;
        S11 = np.copy(S1[0:L,i0])
        H = S11.copy() # initial value
        cost1 = np.zeros(nloop + 1) ; W1=np.empty((nloop, N2)) #; HS = np.empty(N2)
        iloop=1 ; stop_iloop=0
        while iloop < nloop and stop_iloop == 0:
            H_av =np.mean(H)
            HS = np.matmul(dS[0:L,:].T,H-H_av)/L

            W = np.matmul(HS[:], C_inv[:,:])
            H0[iloop] = H_av - np.sum(W*M)

            H[0:L] = H0[iloop] + np.matmul(S[0:L,:], W[:])
            cost1[iloop] = np.mean((S11 - np.tanh(H)) ** 2)
            #MSE = np.mean((W0[:]-W[:])**2)

            if cost1[iloop] > cost1[iloop-1] and iloop > 1:
                stop_iloop = 1
                #print(iloop,i0)
